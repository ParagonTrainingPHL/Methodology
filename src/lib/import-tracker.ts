 ...
 * header rows, and the header wording says which of the two column layouts the
 * rows beneath it use.
 *
 * Callable from the upload page and from scripts/import-xlsx.ts.
 */
...
import { ASSESSMENT_METRICS } from "./constants";
import { suggestBlockKey } from "./classify";
...
async function loadBlocks() {
  // The server process is long-lived; a stale cache would carry ids from a
  // previous import into this one.
  blockCache.clear();
  strengthBlockIds.clear();
...
/** Removes every client and everything hanging off them. Never touches users. */
...
export type ImportSummary = {
  ...
  assessments: number;
  exercises: number;
  inferredBlocks: number;
  perSheet: { sheet: string; kind: "sessions" | "assessments"; count: number }[];
  unmatchedSheets: string[];
};

/**
 * Reads a tracker workbook and writes it to the database.
 *
 * `reset` clears existing client data first, which is what a re-upload of the
 * same workbook means; without it the import adds a second copy of everyone.
 */
export async function importTrackerWorkbook(
  source: Buffer | string,
  options: { reset?: boolean } = {},
): Promise<ImportSummary> {
  if (options.reset) await resetImportedData();
  ...
  if (typeof source === "string") {
    ...
    // ExcelJS types the buffer argument loosely across Node and browser builds.
    await workbook.xlsx.load(source as unknown as ArrayBuffer);
  }
  ...
  for (const sheet of workoutSheets) {
    ...
    sessionCount += count;
    perSheet.push({ sheet: sheet.name, kind: "sessions", count });
  }
  ...
  for (const sheet of assessmentSheets) {
    ...
    perSheet.push({ sheet: sheet.name, kind: "assessments", count });
  }

  const inferredBlocks = await inferMissingBlocks();
